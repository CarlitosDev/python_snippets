...
df_A = df_activities_description[['ActivityTemplate_id', 'TemplateName']].drop_duplicates()
activity_template_map = {row.ActivityTemplate_id: row.TemplateName for row in df_A.itertuples(index=False)}



# Find the index of the value in a column nearest to a given value
# (nearest value within a series)
# use bisect from standard Python
from bisect import bisect_left, bisect_right
idx_potential_begin = df_transcribe.item_begin == idx_begin
idx_begin_this_entity = bisect_left(df_transcribe.item_begin.values, idx_begin)    
print(f'Cannot find {idx_begin} but approximating it to {df_transcribe.item_begin[idx_begin_this_entity]}')





# melt : fix one variable and expand the DF
df = pd.DataFrame({'A': {0: 'a', 1: 'b', 2: 'c'},
                   'B': {0: 1, 1: 3, 2: 5},
                   'C': {0: 2, 1: 4, 2: 6}})
# What are the possible values of B and C depending on the values of A?
# melt: arrange the valies of the numerical variables depending on the 
# values of the response variable. The column variable will contain the name of 
# the variable fixed and its value regarding the target variable.
df.melt('A')



# Merge using the nearest - to review
...
df_buy = df_buy.groupby('Symbol').agg(grp_operations)
df_buy = t_df[t_df.Operation == 'Buy'].groupby('Symbol').agg(grp_operations)
df_buy.columns = ['%s%s' % (a, '_%s' % b if b else '') for a, b in df_buy.columns]
...
# Multiple aggregations on the same variable/column
