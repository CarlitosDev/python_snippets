...
for column in df_students.columns:
  _type = pd.api.types.infer_dtype(df_students[column])
  if _type == 'mixed-integer':
    print(f'Setting {_type} to string')
    df_students[column] = df_students[column].astype(str)
  else:
    print(_type)
...
# Apply lambdas to several columns
stats_to_text = lambda st: f'({st[0]:3.1f},{st[1]:3.1f},{st[2]:3.1f})'
vars_for_stats = ['mean_stores','mean_stores_q25','mean_stores_q75']
df_all_summaries['stores_stats'] = df_all_summaries[vars_for_stats].apply(stats_to_text, axis=1)



# Round decimals in DF columns
...
df_input.rename(columns=str.lower, inplace=True)
df_input.head()


# Apply lambda functions to more than one column / multicolumn
df_cannibal_victim['sku_B_regular'] = df_cannibal_victim[['sales_B_regular_mu', 'sales_B_regular_std']].apply(lambda st: f'Regular {st[0]:3.2f} pm {st[1]:3.2f}', axis=1)
df_cannibal_victim['sku_B_snap'] = df_cannibal_victim[['sales_B_snap_mu', 'sales_B_snap_std']].apply(lambda st: f'Snap {st[0]:3.2f} pm {st[1]:3.2f}', axis=1)
...
# Map values
df_closed_answers.is_correct.map({True: 1, False: 0})
...
# Mind the way str.contains works, especially the options, ie: str.contains('end', na=False, regex=False)
# as it does not always return booleans
import pandas as pd
...
# If timestamps...
text = f'''df = pd.DataFrame.from_dict({d})''' 
...
# Aggregate numbers to operate on them - rolling
...
A bit cooler:
	...
	integerVars     = df.select_dtypes(include=['int']).columns.tolist()
	numericalVars   = df.select_dtypes(exclude=['int', 'object', 'datetime64']).columns.tolist()
