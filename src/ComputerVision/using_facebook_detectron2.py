...
baseFolder = '/Users/carlos.aguilar/Documents/pictures/tester_panoptic'
...
for idx, this_file_path in enumerate(pic_files):
  ...
  outputs = predictor(img_1_data)
  
  panoptic_seg, segments_info = outputs["panoptic_seg"]
  v = Visualizer(img_1_data[:, :, ::-1], MetadataCatalog.get(cfg.DATASETS.TRAIN[0]), scale=1.2)
  out = v.draw_panoptic_seg_predictions(panoptic_seg.to("cpu"), segments_info)

  out_image = out.get_image()[:, :, ::-1]

  output_file = this_file_path.replace('.HEIC', '_panoptic.jpg')
  imgu.save_image(out_image, output_file)
